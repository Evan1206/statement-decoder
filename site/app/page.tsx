...
const cases = [
  ['否定勸退型', '「學 AI 沒用啦！」', '檢查「沒用」的判準與實際證據。'],
  ['年齡／時間焦慮型', '「你不年輕了，要趕快找工作了」', '把模糊焦慮換成真實期限。'],
  ...
  return (
    <main>
      ...
      <section className="cases-section" id="cases">
        ...
        <div className="case-grid">{cases.map(([type, quote, note], index) => <article key={type}><span>0{index + 1}</span><h3>{type}</h3><blockquote>{quote}</blockquote><p>{note}</p></article>)}</div>
      </section>
