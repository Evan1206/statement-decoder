...
async function loadMetrics() {
  try {
    ...
    const totals = await env.DB.prepare(`SELECT
      ...
      SUM(CASE WHEN event_type='analysis_started' AND visitor_id IS NOT NULL THEN 1 ELSE 0 END) tracked_starts,
      SUM(CASE WHEN event_type='analysis_completed' AND visitor_id IS NOT NULL THEN 1 ELSE 0 END) tracked_completions,
      SUM(CASE WHEN event_type='analysis_failed' AND visitor_id IS NOT NULL THEN 1 ELSE 0 END) tracked_failures,
    ...
    const feedbackReasons = await env.DB.prepare(`SELECT event_label label, COUNT(*) count FROM events WHERE event_type='feedback_reason' AND event_label IS NOT NULL GROUP BY event_label ORDER BY count DESC`).all<Row>();
    return { totals, pending: Number(pending?.count ?? 0), categories: categories.results, tags: tags.results, contexts: contexts.results, cross: cross.results, trend: trend.results, feedbackReasons: feedbackReasons.results, ready: true };
  } catch {
    return { totals: {}, pending: 0, categories: [], tags: [], contexts: [], cross: [], trend: [], feedbackReasons: [], ready: false };
  }
}
...
export default async function Dashboard() {
  ...
  const trackedStarts = Number(data.totals?.tracked_starts ?? 0);
  const completionRate = trackedStarts ? Number(data.totals?.tracked_completions ?? 0) / trackedStarts : 0;
  ...
  const maxFeedback = Math.max(1, ...data.feedbackReasons.map((row) => Number(row.count)));
  ...
  return <main className="dashboard">
    ...
    <section className="metric-strip">
      ...
      <article><span>近 30 天匿名裝置</span><strong>{Number(data.totals?.active_devices_30d ?? 0)}</strong><small>不是人數；同一瀏覽器隨機代碼去重</small></article>
      <article><span>解碼完成率</span><strong>{(completionRate * 100).toFixed(0)}%</strong><small>{trackedStarts ? `${Number(data.totals?.tracked_completions ?? 0)} 完成／${trackedStarts} 開始` : '新埋點尚無資料'}</small></article>
      <article><span>解碼失敗</span><strong>{Number(data.totals?.tracked_failures ?? 0)}</strong><small>網路或服務回應失敗</small></article>
    ...
    <section className="dashboard-grid">
      ...
      <article className="chart-card">
        <div><h2>「還可以更好」的原因</h2><p>使用者自願選擇的單一改善方向，不保存原句</p></div>
        <div className="bar-chart tag-chart">{data.feedbackReasons.length ? data.feedbackReasons.map((row) => <div className="bar-row" key={String(row.label)}><span>{row.label}</span><div><i style={{ width: `${Number(row.count) / maxFeedback * 100}%` }} /></div><strong>{row.count}</strong></div>) : <p className="empty">收到改善原因後，這裡會顯示分布。</p>}</div>
      </article>
      <article className="chart-card">
    ...
    <section className="heatmap-card">
      <div><h2>情境 × 話術機制</h2><p>顏色越深，代表該情境下完成解碼的次數越多；1–4 筆一律隱藏為「樣本不足」。</p></div>
      {crossCategories.length && crossContexts.length ? <div className="heatmap-wrap"><table className="heatmap"><thead><tr><th>話術機制</th>{crossContexts.map((context) => <th key={context}>{context}</th>)}</tr></thead><tbody>{crossCategories.map((category) => <tr key={category}><th>{category}</th>{crossContexts.map((context) => { const count = crossCount(category, context); const visible = count >= 5; return <td key={context} title={visible ? `${category} × ${context}：${count}` : count ? '樣本少於 5 筆，已隱藏' : '目前沒有資料'} style={{ backgroundColor: visible ? `rgba(243,111,67,${0.14 + count / maxCross * 0.76})` : undefined }}>{visible ? count : count ? '樣本不足' : '—'}</td>; })}</tr>)}</tbody></table></div> : <p className="empty">累積帶有情境與分類的解碼後，這裡會出現交叉分析。</p>}
    </section>
    <section className="metric-notes"><h2>指標定義</h2><dl><div><dt>匿名裝置</dt><dd>瀏覽器在本站產生的隨機代碼，30 天後到期；它不是登入帳號，也不等於真實人數。清除網站資料、更換瀏覽器或裝置都會重新計算。</dd></div><div><dt>解碼完成率</dt><dd>新埋點上線後，帶匿名代碼的完成事件除以開始事件；舊資料不混入分母。</dd></div><div><dt>話術機制</dt><dd>每次成功解碼的主要分類；一筆解碼只計一個主要機制。</dd></div><div><dt>次要標籤</dt><dd>同一筆解碼可有零至三個標籤，因此不可與解碼總數直接相加比較。</dd></div><div><dt>情境交叉</dt><dd>以完成解碼事件為統計單位，依主要機制與使用者選擇的情境分組；少於 5 筆不顯示精確數量。</dd></div><div><dt>有幫助率</dt><dd>點選「有幫助」次數除以所有正負回饋；沒有回饋時顯示 0%。</dd></div><div><dt>匿名投稿</dt><dd>通過必填與匿名化確認、成功寫入待審核資料表的投稿。</dd></div></dl><p>資料來源：D1 events／submissions 彙總；事件資料不保存分析原句、IP 或瀏覽器指紋。更新時間：{new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' })}</p></section>
  </main>;
}
